...
> Historical manual matrix for the original `/api/v1/*` implementation. The
> current service exposes `/internal/v1/*` with `schemaVersion: "1.1"` and
> response envelopes; use `tests/test_internal_contract.py` and the internal
> contract v1.1 plus the WhatsApp v1.2 addendum for integration acceptance. The old curl samples and expected
> response shapes below are not valid for the current endpoints.
...
- **Endpoint:** POST /api/v1/generate
- **What it tests:** service uses `null` instead of fabricating a realistic-looking number

**Pass criteria:**
- [ ] HTTP 200
- [ ] `contact.whatsappNumber` is exactly `null`
- [ ] `contact.whatsappNumber` is NOT a realistic-looking number (e.g., `"6281234567890"`)

**Fail criteria:**
- HTTP error
- `whatsappNumber` is empty or a masked placeholder
